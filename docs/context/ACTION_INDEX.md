...
| ACT-0016 | 2026-05-09 | opencode_integration | Habilitar Build low-risk real: auto-aprobación explícita de permisos OpenCode + guardrails | in_progress | orchestrator | Build | `scripts/run_opencode_from_handoff.py`; `scripts/create_and_dispatch_opencode_handoff.py`; MCP tool `create_and_dispatch_opencode_handoff` | Objetivo: permitir edición real en modo no interactivo SOLO con `auto_approve_permissions=true` + `build_authorized=true` + `risk_level=low` + `allowed_files` acotado |
