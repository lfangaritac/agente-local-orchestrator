...
| Retoma estratégica por proyecto (session → docs) | `docs/context/REFERENCE_BASED_CONTEXT_PROTOCOL.md` | `mcp_server/tools.py` (tool `sync_active_last_event_to_project_docs`); `docs/projects/<project-id>/PROJECT_RESUME.md`; `docs/projects/<project-id>/CURRENT_FRONTIER.md` | `active_project.last_event` es memoria efímera (gitignored). `PROJECT_RESUME`/`CURRENT_FRONTIER` son versionados. La tool sincroniza **referencias** (no evidencia) y es dry-run por defecto; apply requiere autorización explícita. |
