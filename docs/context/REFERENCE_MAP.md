...
| Compact-first y herramientas MCP | `mcp_server/README.md` | `docs/protocols/MCP_CONTINUE_INTEGRATION_PROTOCOL.md` | Preferir herramientas compactas (`get_run_status`/`check_opencode_run_status`). Evitar `show_latest_run` salvo solicitud explícita o preview-only. |
...
| Retención y versionado de evidencia | `docs/protocols/AGENT_AUTOMATION_PROTOCOL.md` (sección 15.1) | `docs/context/REFERENCE_BASED_CONTEXT_PROTOCOL.md`; `docs/context/RUN_INDEX.md`; `docs/context/ACTION_INDEX.md`; `docs/context/DECISION_INDEX.md`; `.continueignore` | Define qué runs/handoffs/logs/raw_outputs se versionan vs locales/temporales; cuándo actualizar índices; y cómo aplicar exclusiones best-effort sin cargar evidencia completa al contexto. |
