...
def main() -> None:
    ...
    messages = [
        {
            ...
            "params": {
                ...
                "capabilities": {},
                "clientInfo": {"name": "run-general-instruction-flow-test", "version": "0.1.0"},
            },
        },
        {"jsonrpc": "2.0", "method": "notifications/initialized", "params": {}},
        {"jsonrpc": "2.0", "id": 2, "method": "tools/list", "params": {}},
        # plan: sin proyecto -> debe pedir confirmación
        {
            "jsonrpc": "2.0",
            "id": 3,
            "method": "tools/call",
    ...
    for response in responses:
        ...
        if response.get("id") in {3, 4}:
            payload = extract_tool_payload(response)
            if payload is not None:
                payloads[int(response["id"])] = payload

    tools_list_ok = "run_general_instruction_flow" in tool_names

    p3 = payloads.get(3) or {}
    plan_ok = (
        p3.get("mode") == "plan"
        and isinstance(p3.get("plan"), dict)
        and p3["plan"].get("status") == "project_not_confirmed"
        and p3["plan"].get("next_frontier") == "confirm_project"
        and isinstance(p3.get("followup_scheme_template"), dict)
    )

    p4 = payloads.get(4) or {}
    dispatch = p4.get("dispatch") if isinstance(p4.get("dispatch"), dict) else {}
    followup = p4.get("followup_plan") if isinstance(p4.get("followup_plan"), dict) else {}

    dispatch_ok = (
        p4.get("mode") == "dispatch_if_safe"
        and dispatch.get("attempted") is True
        and isinstance(p4.get("run_id"), str)
        and isinstance(followup.get("steps"), list)
        and p4.get("recommended_next_tool_call", {}).get("tool") == "run_health_check"
    )
